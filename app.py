...
client=pymongo.MongoClient("mongodb+srv://walker:walkerm@hod-portal.hpemmk8.mongodb.net/?retryWrites=true&w=majority")
...
def admin():
    ...
    if request.method=="POST":
        ...
        if not len(f):
            ...
            coll.insert_one(rec)
            coll=db["faculty"]
            rec={"name":name,"user_name":user_name,"email":email,"subjects":subjects,"dept":dept}
            coll.insert_one(rec)
            coll=db["faculty_schedule"]
            rec={"user_name":user_name,"mon":[],"tue":[],"wed":[],"thu":[],"fri":[]}
            coll.insert_one(rec)
        else:
            flash("User exists")
    ...
    if request.method=="POST":
        ...
        if password!=conf_password:
            ...
            if not len(f):
                ...
                coll.insert_one(rec)
                rec={"roll":roll,"name":name,"branch":branch,"sem":sem,"email":email,"batch":batch,"country":country,"phone":phone}
                coll=db["student"]
                coll.insert_one(rec)
            else:
                flash("User exists")
    ...
    if request.method=="POST":
        ...
        if f["password"]!=old_pass:
            ...
            if new_pass!=conf_pass:
                ...
                coll.update_one({"roll":roll},{"$set":{"password":new_pass}})
                return redirect(url_for("login"))
    data={"roll":roll}
    ...
    if request.method=="POST":
        ...
        if f["password"]!=old_pass:
            ...
            if new_pass!=conf_pass:
                ...
                coll.update_one({"user_name":user_name},{"$set":{"password":new_pass}})
                return redirect(url_for("login"))
...
def teacher():
    ...
    if request.method=="POST":
        ...
        if slot and day!="-":
            coll=db["student_schedule"]
            coll.update_one({"roll":roll,"user_name":user_name},{"$set":{day:slot}},True)
            coll=db["slots_list"]
            students=list(coll.find({"user_name":user_name,"day":day,"time":slot}))
            if not len(students):
                students=[roll]
            else:
            ...
            coll.update_one({"user_name":user_name,"day":day,"time":slot,"status":"active"},{"$set":{"students":students}},True)
    coll=db["faculty"]
    ...
    data={"roll":roll,"teacher":fac,"t_sched":t_sched,"s_schedule":s_schedule}
...
def status():
    ...
    if request.method=="POST":
        ...
        coll.update_one({"user_name":user_name,"day":day,"time":time},{"$set":{"students":students}})
        coll=db["student_schedule"]
        coll.update_one({"roll":roll,"user_name":user_name},{"$unset":{day:""}})
        ...
        for x in f:
            ...
            for key,value in x.items():
                if key not in ["user_name","roll","_id"]:
                    coll=db["slots_list"]
                    k=list(coll.find({"user_name":user_name,"day":key,"time":value}))
                    if len(k):
                        status=k[0]["status"]
                        students=k[0]["students"]
                        i=students.index(roll)
                        if i>1 and status not in ["cancelled","inactive"]:
...
def select_slot():
        ...
        if request.method=="POST":
            ...
            for time in slot:
                if time in request.form:
                    times.append(time)
            coll.update_one({"user_name":user_name},{"$set":{day:times}},True)
    coll=db["faculty"]
    ...
    avail_days={"-":"-","mon":"Monday","tue":"Tuesday","wed":"Wednesday","thu":"Thursday","fri":"Friday"}
    if today_ not in ["sat","sun"]:
        for a,b in days.items():
            ...
            if a=="-":
                continue
            avail_days.pop(a)
    data={"user_name":user_name,"name":x["name"],"email":x["email"],"dept":x["dept"],"day":y,"avail_days":avail_days,"time_":time_,"today_":today_}
...
def cancel_slot():
    ...
    if request.method=="POST":
        ...
        for x in f:
            time=x["time"]
            if time in slots:
                coll.update_one({"user_name":user_name,"day":day,"time":time},{"$set":{"status":"cancelled"}})
        coll=db["student_schedule"]
        f=list(coll.find({"user_name":user_name,"day":day}))
        for x in f:
            if x[day] in slots:
                coll.update_one({"user_name":user_name,"day":day},{"$unset":{day:""}})
    coll=db["faculty_schedule"]
...
def edit_profile():
    ...
    if request.method=="POST":
        ...
        coll=db["faculty"]
        if name=="":
            coll.update_one({"user_name":user_name},{"$set":{"email":email,"subjects":subjects,"dept":dept}})
        else:
            coll.update_one({"user_name":user_name},{"$set":{"name":name,"email":email,"subjects":subjects,"dept":dept}})
    coll=db["faculty"]
